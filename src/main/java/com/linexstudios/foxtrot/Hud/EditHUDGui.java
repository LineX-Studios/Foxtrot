...
public class EditHUDGui extends GuiScreen {
    ...
    @Override public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        ...
        if (panelCollapsed) {
            ...
            if (selectedTab == 0) {
                ...
                drawSettingsCard(c1, y1 += 22, 135, ConfigHandler.showFoxtrotCapes ? 52 : 34); fontRendererObj.drawStringWithShadow(EnumChatFormatting.BOLD + "Cosmetics", c1 + 5, y1 + 5, -1); drawIOSToggle(c1 + 5, y1 += 18, 125, "Foxtrot Capes", ConfigHandler.showFoxtrotCapes, mX, mY); if (ConfigHandler.showFoxtrotCapes) drawIOSButton(c1 + 5, y1 += 16, 125, 14, "Cape: " + ConfigHandler.selectedCape, mX, mY);
                ...
                drawIOSButton(mainPanelX + 155, mainPanelY + 185, 160, 16, "Reset HUD Positions", mX, mY);
        ...
        if (b == 0) {
            if(panelCollapsed) { if(isInside(mX, mY, collapsedX + 95, collapsedY, 20, 18)) { panelCollapsed=false; return; } if(isInside(mX, mY, collapsedX, collapsedY, 95, 18)) { draggingPanel=true; lastX=mX; lastY=mY; return; } } 
                ...
                if(selectedTab==0) {
                    ...
                    if(isInside(mX, mY, c1+5, y1+=36, 125, 12)) ConfigHandler.showFoxtrotCapes=!ConfigHandler.showFoxtrotCapes;
                    if(ConfigHandler.showFoxtrotCapes && isInside(mX, mY, c1+5, y1+=16, 125, 14)) { int idx = CapeManager.availableCapes.indexOf(ConfigHandler.selectedCape); ConfigHandler.selectedCape = CapeManager.availableCapes.get((idx + 1) % CapeManager.availableCapes.size()); }
                    ...
                    if(isInside(mX, mY, mainPanelX + 155, mainPanelY + 185, 160, 16)) { for(DraggableHUD h : DraggableHUD.getRegistry()) h.resetPosition(); ConfigHandler.saveConfig(); }
                } else if(selectedTab==4) {
    ...
    private void drawIOSToggle(float x, float y, float cW, String l, boolean o, int mX, int mY) { 
        fontRendererObj.drawStringWithShadow(l, x, y+2, 0xDDDDDD); 
        float sX = x+cW-28, sY = y+1; 
        RenderUtils.drawRoundedRect(sX, sY, 24, 12, 6, o?COLOR_ENABLED:(isInside(mX,mY,x,y,cW,14)?0xFF555555:0xFF444444)); 
        RenderUtils.drawCircle(o?sX+18:sX+6, sY+6, 5, 0xFFFFFFFF); 
    }
    ...
    private boolean isInside(float mX, float mY, float x, float y, float w, float h) { return mX>=x && mX<=x+w && mY>=y-2 && mY<=y+h+2; }
    @Override
