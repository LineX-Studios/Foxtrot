...
public class EnemyHUD extends DraggableHUD {
    ...
    public void draw(boolean isEditing) {
        ...
        this.width = locX + (rows.get(0).loc.isEmpty() ? 0 : fr.getStringWidth("- " + rows.get(0).loc));
        // Recalculate true width
        for (HUDRow row : rows) {
            int w = locX + (row.loc.isEmpty() ? fr.getStringWidth("- " + row.gear) - 10 : fr.getStringWidth("- " + row.loc));
